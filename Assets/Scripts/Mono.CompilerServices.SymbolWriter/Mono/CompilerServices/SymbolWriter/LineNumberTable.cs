	...
	public class LineNumberTable
	{
		...
		internal void Write(MonoSymbolFile file, MyBinaryWriter bw)
		{
			...
			for (int i = 0; i < LineNumbers.Length; i++)
			{
				...
				if (i + 1 < LineNumbers.Length && LineNumbers[i + 1].Equals(LineNumbers[i]))
				{
					continue;
				}
		...
		internal static LineNumberTable Read(MonoSymbolFile file, MyBinaryReader br)
		{
			...
			lineNumberTable.DoRead(file, br);
		...
		private void DoRead(MonoSymbolFile file, MyBinaryReader br)
		{
			...
			while (true)
			{
				...
				{
					byte b2 = br.ReadByte();
					long position = br.BaseStream.Position + (int)b2;
					b = br.ReadByte();
					switch (b)
					{
					case 1:
						if (flag2)
						{
							arrayList.Add(new LineNumberEntry(file2, num, num2, flag));
						}
						_line_numbers = new LineNumberEntry[arrayList.Count];
						arrayList.CopyTo(_line_numbers, 0);
						return;
					case 64:
						flag = !flag;
						flag2 = true;
						break;
					default:
						throw new MonoSymbolFileException("Unknown extended opcode {0:x} in LNT ({1})", b, file.FileName);
					case 65:
					case 66:
					case 67:
					case 68:
