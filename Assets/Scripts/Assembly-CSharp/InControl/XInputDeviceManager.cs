...
namespace InControl
{
	public class XInputDeviceManager : InputDeviceManager
	{
		...
		private RingBuffer<GHMBOFCIIJJ>[] gamePadState = new RingBuffer<GHMBOFCIIJJ>[4];
		...
		public XInputDeviceManager()
		{
			...
			for (int i = 0; i < 4; i++)
			{
				gamePadState[i] = new RingBuffer<GHMBOFCIIJJ>(bufferSize);
			}
			StartWorker();
			for (int j = 0; j < 4; j++)
			{
		...
		private void Worker()
		{
			while (true)
			{
				for (int i = 0; i < 4; i++)
				{
					gamePadState[i].Enqueue(NLIKGPCKJBM.GetState((LKIHIADKFOI)i));
				}
				Thread.Sleep(timeStep);
			}
		}

		internal GHMBOFCIIJJ GetState(int deviceIndex)
		{
		...
		public static bool CheckPlatformSupport(ICollection<string> errors)
		{
			...
			try
			{
				NLIKGPCKJBM.GetState(LKIHIADKFOI.One);
			}
