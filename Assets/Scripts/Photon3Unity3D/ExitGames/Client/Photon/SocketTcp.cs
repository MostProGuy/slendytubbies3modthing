			...
			catch (Exception)
			{
				if (base.State != PhotonSocketState.Disconnecting && base.State != 0)
				{
					if (ReportDebugOfLevel(DebugLevel.ERROR))
					{
						...
						if (sock != null)
						{
							text = string.Concat(" Local: ", sock.LocalEndPoint, " Remote: ", sock.RemoteEndPoint, " (", sock.Connected ? "connected" : "not connected", ", ", sock.IsBound ? "bound" : "not bound", ")");
						}
						EnqueueDebugReturn(DebugLevel.ERROR, "Cannot send to: " + base.ServerAddress + ". Uptime: " + (SupportClass.GetTickCount() - peerBase.timeBase) + "ms." + (base.AddressResolvedAsIpv6 ? " IPv6" : "") + text);
					}
