	...
	public class WallHackDetector : ACTkDetectorBase
	{
		...
		private static WallHackDetector GetOrCreateInstance
		{
			get
			{
				...
				Instance = ACTkDetectorBase.detectorsContainer.AddComponent<WallHackDetector>();
				return Instance;
			}
		}
			...
			if (base.enabled && base.gameObject.activeSelf)
			{
				...
				if (checkWireframe && wfCamera == null)
				{
					...
					else
					{
						...
						{
							shaderTexture = new Texture2D(4, 4, TextureFormat.RGB24, false, true);
						...
						{
							targetTexture = new Texture2D(4, 4, TextureFormat.RGB24, false, true);
