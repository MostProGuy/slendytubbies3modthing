...
public static class PhotonNetwork
{
	...
	static PhotonNetwork()
	{
		...
		if (UsePreciseTimer)
		{
			...
			networkingPeer.LocalMsTimestampDelegate = _003CPhotonNetwork_003Em__0;
		}
	...
	{
		string @string = ObscuredPrefs.GetString("ServerID");
		if (ObscuredPrefs.GetString("ServerType") == "APP")
		{
			PhotonServerSettings.HostType = ServerSettings.HostingOption.PhotonCloud;
		}
		if (ObscuredPrefs.GetString("ServerType") == "IP")
		{
			PhotonServerSettings.HostType = ServerSettings.HostingOption.SelfHosted;
		}
		if (networkingPeer.PeerState != 0)
		{
		...
		networkingPeer.SetApp(@string, gameVersion);
		...
		if (PhotonServerSettings.HostType == ServerSettings.HostingOption.SelfHosted)
		{
			...
			networkingPeer.MasterServerAddress = ((PhotonServerSettings.ServerPort != 0) ? (@string + ":" + PhotonServerSettings.ServerPort) : @string);
	...
	[CompilerGenerated]
	private static int _003CPhotonNetwork_003Em__0()
	{
		return (int)startupStopwatch.ElapsedMilliseconds;
	}
}
